...
def tmp_repo(tmp_path):
    ...
    workspace = tmp_path / "ws-affinity"
    workspace.mkdir()
    (workspace / "_meta.yml").write_text("title: Affinity\n", encoding="utf-8")
    ...
    (ws_dir / "remote-workspaces.yml").write_text(
        yaml.dump({"affinity": {"path": str(workspace)}}), encoding="utf-8"
    )
...
class TestResolveWorkspaceRoot:
    def test_default_root_returns_workspace(self, tmp_repo, monkeypatch):
        ...
        result = _resolve_workspace_root("affinity", Path("."), tmp_repo)
        assert result == (tmp_repo / "ws-affinity").resolve()

    def test_subdir_returns_workspace_subpath(self, tmp_repo, monkeypatch):
        monkeypatch.chdir(tmp_repo)
        result = _resolve_workspace_root("affinity", Path("products/nova"), tmp_repo)
        assert result == (tmp_repo / "ws-affinity" / "products" / "nova").resolve()
    ...
    def test_missing_subdir_errors(self, tmp_repo, monkeypatch):
        ...
        with pytest.raises(click.UsageError, match="not found in workspace"):
            _resolve_workspace_root("affinity", Path("does/not/exist"), tmp_repo)
    ...
    def test_traversal_blocked(self, tmp_repo, monkeypatch):
        ...
        with pytest.raises(click.UsageError, match="escapes workspace"):
            _resolve_workspace_root("affinity", Path("../.."), tmp_repo)
...
class TestLintWithWorkspaceSubdir:
    def test_lint_workspace_root(self, tmp_repo, monkeypatch):
        monkeypatch.chdir(tmp_repo)
        runner = CliRunner()
        result = runner.invoke(main, ["lint", "-w", "affinity"])
    ...
    def test_lint_workspace_subdir(self, tmp_repo, monkeypatch):
        monkeypatch.chdir(tmp_repo)
        runner = CliRunner()
        result = runner.invoke(main, ["lint", "-w", "affinity", "products/nova"])
        assert result.exit_code == 0, f"failed: {result.output}"
        # Confirmation line includes the subdir
        assert "affinity/products/nova" in result.output

    def test_lint_workspace_missing_subdir(self, tmp_repo, monkeypatch):
        monkeypatch.chdir(tmp_repo)
        runner = CliRunner()
        result = runner.invoke(main, ["lint", "-w", "affinity", "fake/path"])
